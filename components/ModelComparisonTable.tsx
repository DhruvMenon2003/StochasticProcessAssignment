...
const metricLabels: { [key: string]: string } = {
  ...
  'Avg Hellinger Distance': 'Avg Hellinger Distance',
};
        ...
        <tbody className="font-mono">
          {metrics.map(metricKey => (
            <tr key={metricKey} className="border-b border-gray-700 last:border-b-0">
              <td className="p-3 font-semibold text-gray-400">{metricLabels[metricKey] || metricKey}</td>
              {modelResults.map(res => {
                ...
                return (
                  <td key={res.name} className={`p-3 text-center ${metric?.isWinner ? 'bg-green-900/40' : ''}`}>
                    <div className="flex items-center justify-center gap-2">
                      <span>{typeof metric?.value === 'number' && isFinite(metric.value) ? metric.value.toFixed(5) : 'N/A'}</span>
